...
@section('title', 'Edit ' . $task->name . ' task - ' . env('APP_NAME'))
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-8">
                            <h4 class="header-title mb-3">Edit task
                                "{{ Str::limit($task->name, 100, '...') }}"
                            </h4>
                        </div>
                        <div class="col-4 text-end">
                            <span class="badge bg-primary p-1">{{ $task->user->name }}</span>
                        </div>
                    </div>
                    <form action="{{ route('tasks.update', ['task' => $task->id]) }}" enctype="multipart/form-data"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-3 col-sm-12 mb-3">
                                            ...
                                            <select class="form-select" id="priority" name="priority">
                                                <option value="low" @if ($task->priority == 'low') selected @endif>Low
                                                </option>
                                                <option value="medium" @if ($task->priority == 'medium') selected @endif>
                                                    Medium</option>
                                                <option value="high" @if ($task->priority == 'high') selected @endif>
                                                    High</option>
                                                <option value="very high" @if ($task->priority == 'very high') selected @endif>
                                                    Very High</option>
                                            </select>
                                        ...
                                        <div class="col-lg-4 col-sm-12 mb-3">
                                            ...
                                            <select class="form-control" name="depenend" data-toggle="select2">
                                                ...
                                                @foreach ($tasks as $tk)
                                                    <option value="{{ $tk->id }}"
                                                        @if ($tk->id == $task->depenend) selected @endif>
                                                        {{ $tk->name }}</option>
                                                @endforeach
                                                {{-- </optgroup> --}}
                                            </select>
                                            ...
                                            <select class="form-control select2-multiple" name="users_assigned[]"
                                                ...
                                                @foreach ($users as $user)
                                                    <option value="{{ $user->id }}"
                                                        @if (in_array($user->id, $assigned_users)) selected @endif>
                                                        {{ $user->name }}</option>
                                                @endforeach
                                                {{-- </optgroup> --}}
                                            </select>
                                            @if ($errors->has('users_assigned'))
                                                <p class="text-pink mt-2">
                                                    {{ $errors->first('users_assigned') }}
                                                </p>
                                            @endif
                                        </div> <!-- end col -->
